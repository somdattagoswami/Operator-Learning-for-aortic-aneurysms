...
Before running the code: Provide the path for the training and testing dataset in dataset.py Line: 26 and 27 respectively.   
'''
import os
...
num = 9
#branch net
layer_B1 = [num, 128, 128, 128, p]
layer_B2 = [1, 128, 128, 128, p]
layer_B3 = [num, 128, 128, 128, p]
layer_B4 = [1, 128, 128, 128, p]
#trunk net
...
def main(save_index):
    data = DataSet(nx, bs)
    x_train, f1_train, f2_train, f3_train, f4_train, u_train, Xmin, Xmax = data.minibatch()

    f1_ph = tf.placeholder(shape=[None, 1, num], dtype=tf.float32)
    f2_ph = tf.placeholder(shape=[None, 1, 1], dtype=tf.float32)
    f3_ph = tf.placeholder(shape=[None, 1, num], dtype=tf.float32)
    f4_ph = tf.placeholder(shape=[None, 1, 1], dtype=tf.float32)
    ...
    W_B3, b_B3 = fnn_model.hyper_initial(layer_B3)
    u_B3 = fnn_model.fnn_B(W_B3, b_B3, f3_ph)  

    W_B4, b_B4 = fnn_model.hyper_initial(layer_B4)
    ...
    #inner product    
    u_nn = tf.einsum('ijk,mk->imj',u_B,u_T)
    ...
    fnn_model.l2_regularizer(W_B3) + fnn_model.l2_regularizer(W_B4)
    ...
    while n <= epochs:
        ...
        if (epochs <= 300000): 
            ...
            lr = 0.0005            
        x_train, f1_train, f2_train, f3_train, f4_train, u_train, _, _ = data.minibatch() 

        train_dict={f1_ph: f1_train, f2_ph: f2_train, f3_ph: f3_train, f4_ph: f4_train, u_ph: u_train[:,:,0:1], x_ph: x_train, learning_rate: lr}
        loss_,_ = sess.run([loss, train_adam], feed_dict=train_dict)
        loss_=sess.run(loss, feed_dict=train_dict)    
        train_loss[n,0] = loss_
        
        if n%1000 == 0:
            x_test, f1_test, f2_test, f3_test, f4_test, u_test = data.testbatch(bs_test)
            u_test_ = sess.run(u_pred, feed_dict={f1_ph: f1_test, f2_ph: f2_test, f3_ph: f3_test, f4_ph: f4_test, x_ph: x_test})
    ...
    x_train, f1_train, f2_train, f3_train, f4_train, u_train, _, _ = data.alldata()
    train_dict={f1_ph: f1_train, f2_ph: f2_train, f3_ph: f3_train, f4_ph: f4_train, u_ph: u_train[:,:,0:1], x_ph: x_train, learning_rate: lr}
    ...
    data_save = SaveData()
    data_save.save(sess, fnn_model, W_T, b_T, W_B1, b_B1, W_B2, b_B2, W_B3, b_B3, W_B4, b_B4, Xmin, \
                   Xmax, f1_ph, f2_ph, f3_ph, f4_ph, u_ph, x_ph, data, num_test, num_noisy, save_results_to)
