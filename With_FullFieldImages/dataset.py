...
class DataSet:
    ...
    def load_data(self):
        ...
        data_train = io.loadmat('./Data/aortic_aneurysm_qoi_trainBW_SD')
        data_test = io.loadmat('./Data/aortic_aneurysm_qoi_testBW_SD')
        ...
        f1_test = f1_test[:,::r,::r]
        f2_test = f2_test[:,::r,::r]
        f1_noisy = f1_noisy[:,::r,::r]
        f2_noisy = f2_noisy[:,::r,::r]
        ...
        f1_train_mean = np.mean(np.reshape(f1_train, (-1, s1, s2, 1)), 0)
        f1_train_std = np.std(np.reshape(f1_train, (-1, s1, s2, 1)), 0)
        f1_train_mean = np.reshape(f1_train_mean, (-1, s1, s2,1))
        F1_train = np.reshape(f1_train, (-1, s1, s2, 1))
        F1_train = (F1_train - f1_train_mean)/(f1_train_std + 1.0e-9) + 3.0

        f2_train_mean = np.mean(np.reshape(f2_train, (-1, s1, s2, 1)), 0)
        f2_train_std = np.std(np.reshape(f2_train, (-1, s1, s2, 1)), 0)
        f2_train_mean = np.reshape(f2_train_mean, (-1, s1, s2, 1))
        F2_train = np.reshape(f2_train, (-1, s1, s2, 1))
        F2_train = (F2_train - f2_train_mean)/(f2_train_std + 1.0e-9) + 3.0
        
        F1_test = np.reshape(f1_test, (-1, s1, s2, 1))
        F1_test = (F1_test - f1_train_mean)/(f1_train_std + 1.0e-9) + 3.0
        F2_test = np.reshape(f2_test, (-1, s1, s2, 1))
        F2_test = (F2_test - f2_train_mean)/(f2_train_std + 1.0e-9) + 3.0

        F1_noisy = np.reshape(f1_noisy, (-1, s1, s2, 1))
        F1_noisy = (F1_noisy - f1_train_mean)/(f1_train_std + 1.0e-9) + 3.0
        F2_noisy = np.reshape(f2_noisy, (-1, s1, s2, 1))
